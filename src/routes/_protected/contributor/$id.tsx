...
function RouteComponent() {
  ...
  const filteredWordBank = searchQuery
    ? listVocabulariesQuery.data?.filter(
        (word) =>
          word.vocab_id.toLowerCase().includes(searchQuery.toLowerCase()) ||
          word.vocab_kr.toLowerCase().includes(searchQuery.toLowerCase())
      )
    : listVocabulariesQuery.data;
  ...
  const groupedWords = filteredWordBank?.reduce(
    (acc, word) => {
      ...
      if (filter === "id") {
        firstLetter = word.vocab_id[0]?.toLowerCase();
      } else {
        const firstChar = word.vocab_kr[0];
  ...
  const deleteVocabulariesMutation = useMutation({
    mutationFn: async (id: string) => {
      const response = await pb.collection("vocabularies_data").delete(id);
      return response;
    },
    onError: (error) => {
      console.error(error);
  ...
  if (listVocabulariesQuery.isLoading) {
    return (
      <div>
        <span>Loading...</span>
      </div>
  ...
  if (listVocabulariesQuery.isError) {
    return (
      <div>
        <span>{listVocabulariesQuery.error.message}</span>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col w-full">
      ...
      <div className="flex flex-col flex-1 p-15 gap-5 max-w-1/2 mx-auto">
          ...
          {groupedWords &&
            filteredLetters.map((letter) => {
              ...
              const sortedWords = [...wordsInGroup].sort((a, b) => {
                if (filter === "id") {
                  return a.vocab_id.localeCompare(b.vocab_id);
                } else {
                  return a.vocab_kr.localeCompare(b.vocab_kr, "ko");
                }
              });
                  ...
                  <div>
                    {sortedWords.map((word) => (
                      ...
                      >
                        {filter === "id" && (
                          <>
                            <span>{word.vocab_id}</span>
                            <span>{word.vocab_kr}</span>
                          </>
                        )}
                        {filter === "kr" && (
                          <>
        ...
        {showForm && listVocabulariesQuery.data && (
          <FormModal
            onClose={() => {
              setShowForm(false);
            ...
            id={selectedWordBank}
            wordBank={listVocabulariesQuery.data}
            onDelete={(id, form) => {
              if (id) {
                deleteVocabulariesMutation.mutate(id);
                setShowForm(false);
                setSelectedWordBank(undefined);
              }
              form?.reset();
    ...
    if (props.id) {
      ...
      if (selectedWordBank) {
        if (textIdRef.current) {
          textIdRef.current.value = selectedWordBank.vocab_id;
        }
        if (textKrRef.current) {
          textKrRef.current.value = selectedWordBank.vocab_kr;
        }
        if (notesRef.current) {
